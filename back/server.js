...
const difficultySettings = {
  easy: { successRate: 0.7, thinkingTime: 1500 },
  medium: { successRate: 0.9, thinkingTime: 1000 },
  hard: { successRate: 1.0, thinkingTime: 500 },
};
let usedWords = [];
...
app.get('/api/start', (req, res) => {
  ...
  usedWords.push(firstWord.word);
  res.json(firstWord);
});

app.post('/api/turn', (req, res) => {
  const { word: playerWord, difficulty = 'medium' } = req.body;
  ...
  setTimeout(() => {
    ...
    res.json({ isValid: true, playerWordDescription: foundPlayerWord.description, ...cpuWordData });
